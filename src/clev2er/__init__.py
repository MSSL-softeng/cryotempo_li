...
- prepares **activate.sh**, the script used to activate the CryoTEMPO
  virtual environment and set environment variables. This script is part
  of the repository, so it is not generated by the install.
...
**Note** that if sourcing **activate.sh** reports that paths pointed to by
environment variables do not exist, you can override any of them by
exporting the variable before you source the script, eg:

```
export CPDATA_DIR=/my/local/cpdata
source ./activate.sh
```
...
within your current shell. The repository provides a script to do this
for you. Run this each time you start a new shell.
...
**Note**: make sure you have activated the software (using **source activate.sh**)
...
contain LRM,SIN/<YYYY>/<MM>/). L1B_BASE_DIR is set in the activate.sh
