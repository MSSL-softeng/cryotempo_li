...
class Algorithm(BaseAlgorithm):
    ...
    def process(self, l1b: Dataset, shared_dict: dict) -> Tuple[bool, str]:
        ...
        **IMPORTANT NOTE:** when logging within the Algorithm.process() function you must use
        ...
        if "height_20_ku_2" in shared_dict:
            shared_dict["height_filt_2"] = shared_dict["height_20_ku_2"].copy()
        else:
            return (True, "")
        ...
        elevation_outliers = np.where(
            np.abs(shared_dict["height_20_ku_2"] - shared_dict["dem_elevation_values"]) > max_diff
        )[0]
        if elevation_outliers.size > 0:
            shared_dict["height_filt_2"][elevation_outliers] = np.nan
            self.log.info(
        ...
        elevation_outliers = np.where(
            (shared_dict["height_20_ku_2"] > max_elevation)
            | (shared_dict["height_20_ku_2"] < min_elevation)
        )[0]
        if elevation_outliers.size > 0:
            shared_dict["height_filt_2"][elevation_outliers] = np.nan
            self.log.info(
