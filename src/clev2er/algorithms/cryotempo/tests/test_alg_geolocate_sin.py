"""pytest of algorithm
   clev2er.algorithms.cryotempo.alg_geolocate_sin.py
"""
...
from clev2er.algorithms.cryotempo.alg_geolocate_sin import Algorithm
...
def test_alg_geolocate_sin(l1b_file, l2i_file) -> None:
    ...
    if shared_dict["instr_mode"] == "LRM":
        assert success, "Should succeed but do nothing with LRM file"
    if shared_dict["instr_mode"] == "SIN":
        ...
        if shared_dict["instr_mode"] == "LRM":
            assert np.abs(mean_height_diffs) < 1.0
            assert stdev_height_diffs < 3.0
        if shared_dict["instr_mode"] == "SIN":
