...
from clev2er.utils.cs2.geolocate.get_heading import get_heading
...
def geolocate_roemer_sar(
    ...
    along_track_beam_width = config["instrument"]["along_track_beam_width_sar"]  # meters

    across_track_doppler_footprint_width = config["instrument"][
        "across_track_doppler_footprint_width"
    ]  # m
    reference_bin_index = config["instrument"]["ref_bin_index_sar"]
    range_bin_size = config["instrument"]["range_bin_size_sar"]  # meters
    num_bins = config["instrument"]["num_range_bins_sar"]
    ...
    return (height_20_ku, lat_poca, lon_poca, slope_ok, relocation_distance)
